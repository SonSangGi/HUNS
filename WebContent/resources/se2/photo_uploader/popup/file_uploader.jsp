...
String path = "C:/web_workspace/shopping/WebContent/images/items"; // 이미지가 저장될 주소
String path2 = request.getSession().getServletContext().getRealPath("/") + File.separator + "/images/items";
String filename = "";
...
<%
	...
	try {
		...
		if (!upfile.equals("")) {
			...
			System.out.println("moveFileName : " + moveFileName);
			File targetFile2 = new File(path2 + File.separator + moveFileName);
			sourceFile.delete();
			...
			try {
				...
				// 복사된 파일의 위치를 지정해준다.
				output = new FileOutputStream(new File(request.getSession().getServletContext().getRealPath("/") + File.separator + "/images/items/"+filename));
